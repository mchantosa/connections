...
module.exports = class ConnectionsDB {
  ...
  async getUserData() {
    if (!this.user) throw new Error('A session is required to access  getUserData');
  ...
  async updateUserData(firstName, lastName) {
    if (!this.user) throw new Error('A session is required to access  updateUserData');
  ...
  async getContact(contactId) {
    ...
    if (result.rowCount > 0) {
      return await this.packageContact(result.rows[0]);
    }
    return false;
  }
