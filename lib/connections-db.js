...
module.exports = class ConnectionsDB {
  ...
  async getUserCredentials(usernameOrEmail) {
    const FIND_USER_CREDENTIALS = 'SELECT id, username, email FROM users WHERE LOWER(username)=LOWER($1) OR LOWER(email)=LOWER($1)';
  ...
  async existsUsername(username) {
    const FIND_USERNAME = 'SELECT username FROM users WHERE LOWER(username) = LOWER($1)';
    const result = await dbQuery(FIND_USERNAME, username);
    return result.rowCount > 0;
  }

  async existsEmail(email) {
    const FIND_EMAIL = 'SELECT email FROM users WHERE LOWER(email) = LOWER($1)';
  ...
  async getContactsNames(query) {
    if (!this.user) throw new Error('A session is required to access  getContactsNames');

    const FIND_CONTACTS = `
    SELECT
      id,
      CONCAT_WS(' ', first_name, last_name) AS name,
      LOWER(CONCAT_WS(' ', first_name, last_name)) AS sorted_name
    FROM
      contacts
    WHERE
      user_id = $1
      AND LOWER(CONCAT_WS(' ', first_name, last_name)) LIKE LOWER(CONCAT('%', $2::text, '%'))
    ORDER BY
      sorted_name`;
    const result = await dbQuery(FIND_CONTACTS, this.user.id, query);
  ...
  async getContactsAll(date) {
    ...
    const GET_CONTACTS = `SELECT contacts.id,
      ...
      ORDER  BY objectives.next_contact_date ASC, 
        ...
        TRIM(CONCAT_WS(' ', LOWER(contacts.first_name), LOWER(contacts.last_name)));`;
  ...
  async getContactId(contactName) {
    if (!this.user) throw new Error('A session is required to access  getContactId');

    const GET_CONTACT = `
      SELECT id
      FROM contacts 
      WHERE user_id = $1
      AND LOWER(TRIM(CONCAT_WS(' ', first_name, last_name))) = LOWER($2);`;
    const result = await dbQuery(GET_CONTACT, this.user.id, contactName);
